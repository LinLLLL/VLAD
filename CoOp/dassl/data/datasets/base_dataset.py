...
class Datum:
    ...
    def __init__(self, impath="", label=0, domain=0, classname="", domainname="", alldomain=None):
...
class DatasetBase:
    ...
    def __init__(self, train_x=None, train_u=None, val=None, test=None, train_samples=None, alldomain=None):
        ...
        self._num_classes = self.get_num_classes(train_x+test)
        self._lab2cname, self._classnames, self._domainnames = self.get_lab2cname(train_x+test)
        self._lab2cname, self._classnames_o, self._domainnames = self.get_lab2cname(train_x)
        self._lab2cname, self._classnames_t, self._domainnames = self.get_lab2cname(test)
    ...
    @property
    def classnames_o(self):
        return self._classnames_o

    @property
    def classnames_t(self):
        return self._classnames_t
    ...
    def generate_fewshot_dataset_balance(
            self, *data_sources, num_shots=-1, repeat=False
    ):
        """Generate a few-shot dataset (typically for the training set).
        ...
        This function is useful when one wants to evaluate a model
        in a few-shot learning setting where each class only contains
        a few number of images. 为每类都抽取小样本, 且每个环境的样本数量相同
        ...
        for data_source in data_sources:
            ...
            for label, items in tracker.items():
                if len(items) >= num_shots:
                    tracker_d = self.split_dataset_by_domain(data_source)
                    num_shots_d = int(num_shots / len(tracker_d.items()))
                    # print(f"Creating a {num_shots_d}-shot dataset")
                    for label_d, items_d in tracker_d.items():
                        if len(items_d) >= num_shots_d:
                            sampled_items_d = random.sample(items_d, num_shots_d)
                        else:
                            if repeat:
                                sampled_items_d = random.choices(items_d, k=num_shots_d)
                            else:
                                sampled_items_d = items_d
                        dataset.extend(sampled_items_d)
                else:
                    if repeat:
                        sampled_items = random.choices(items, k=num_shots)
                    else:
