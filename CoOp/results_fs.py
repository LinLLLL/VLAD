...
ACC_all = {}
# for domain in ["test"]:
#     root = '/data_disk/zl/IJCV_Rebuttal/CelebA_2/FewShot_ViT/' + domain + '/VLAD/'
for domain in ["ColoredCatsDogs.json"]:
    root = '/data_disk/zl/IJCV_Rebuttal_124816/ColoredCatsDogs_-1/FewShot_ViT/' + domain + '/VLAD/'
    key = domain[:-4].split('_')[-1]
    ACC = {}
    P0 = os.listdir(root)
    for p0 in P0:
        key0 = p0.split('seed')[1][0]
        ACC[key0] = {}
        path1 = os.path.join(root, p0)
        if True:
            key1 = p0.split('seed')[1][1:]  # hps
            ACC[key0][key1] = []
...
for idx, domain in enumerate(ACC_all.keys()):
    ...
    for seed in ACC_all[domain].keys():
        ...
        for alpha in ACC_all[domain][seed].keys():
            c = []
            if alpha.split('_')[-1] == '16' and alpha.split('_')[1] != '2.0':
            # if True:
...
for seed in range(3):
    ...
    for i in range(len(val[0][0])):
        ...
        print(keylist[i][1:], np.mean([test[d][0][i] for d in range(1)]), np.mean([test_ACC_S[d][0][i] for d in range(1)]), np.mean([test_FPR[d][0][i] for d in range(1)]))
