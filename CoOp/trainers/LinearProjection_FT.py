...
class LinearProjection_FT(TrainerX):
    ...
    def build_model(self):
        ...
        nfeat = 1024 if cfg.MODEL.BACKBONE.NAME == "RN50" else 512
        nfeat = 768 if cfg.MODEL.BACKBONE.NAME == "ViT-L-14" else nfeat
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            # load the pretrained LP weights
            load_pretrained_weights(self.model, cfg.MODEL.INIT_WEIGHTS)

        self.model.to(self.device)
        self.optim = build_optimizer(self.model, cfg.OPTIM) 
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("full", self.model, self.optim, self.sched) 
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.LinearProjection.PREC
