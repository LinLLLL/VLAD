...
for t in plist:
    ...
    elif "ColoredCatsDogs" in t:
        domains = ['ColoredCatsDogs.json']
    elif "CelebA" in t:
        domains = ['test']
    else:
        raise AssertionError(
            "this file in only used for collect results for OOD datasets: PACS, VLCS, ColoredCatesDogs and CelebA")
    plists.append(t)
...
for file in plists:
    ...
    for domain in ACC_all.keys():
        for seed in ACC_all[domain].keys():
            if len(ACC_all[domain][seed]['16']) >= 1:
                test_acc.append(ACC_all[domain][seed]['16'][0])
                val_acc.append(ACC_all[domain][seed]['16'][1])
            else:
                test_acc.append(0)
                val_acc.append(0)
